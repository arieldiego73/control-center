...
const BusinessUnitTable: React.FC<DataGridProps> = (props) => {
	...
	const data = useSelector((state: RootState) => state.devTypeReducer.devType);
	...
	const devTypeRef = React.useRef<HTMLInputElement | null>(null);
	...
	const addRecord = (isAddOnly: boolean) => {
		if (devTypeName && shortName) {
			const devTypeData: GridValidRowModel = {
				dev_type_name: devTypeName,
				dev_type_sh_name: shortName,
			};
			processAddRow(devTypeData);
			setDevTypeName("");
			setShortName("");
			if (isAddOnly) {
				setIsHidden(false);
