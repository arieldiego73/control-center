...
function* loginSaga(action: ReturnType<typeof login>): any {
  try {
    ...
    if (user) {
      ...
      // yield put(setAuthenticationStatus(localStorage.setItem("isAuthenticated", "true")));
    ...
    //   cookies.set('isAuthenticated', 'false', { path: '/' });
      ...
      // Set the localStorage value
        ...
        // yield put(setAuthenticationStatus(localStorage.setItem("isAuthenticated", "false")));
...
function* logoutSaga(): any {
    ...
    yield put(clearUser());
    // cookies.remove('isAuthenticated', { path: '/' }); // Make sure the name and path match the login cookie
